...
| 서비스 | 관광지 종류 큐레이션 | 위치 기반 오디오 | 혼잡도 최적화 | 경비 자동합산 |
|---|:-:|:-:|:-:|:-:|
| 트리플 | △ | ✗ | ✗ | ✗ |
| 야놀자 | △ | ✗ | ✗ | △ |
| 마이리얼트립 | △ | ✗ | ✗ | △ |
...
핵심 3대 차별점: (1) **분위기 보존 분산** — 바이럴 장소를 포기시키지 않고 분위기가 91% 이상 유사한 대안을 같은 카테고리 맥락으로 제시한다. (2) **실행 가능한 원페이지** — 지도 경로·예약 안내·경비 합산·오디오 안내가 결합된 단일 화면을 제공한다. (3) **걸으면 따라오는 이야기** — 문화유산 50m 진입 시 5개국어 오디오가 자동 재생되어 별도 조작 없이 외국인 개별 여행객을 포괄한다.
...
![한국관광공사 공공데이터 연계 구조도](assets/architecture-diagram.svg)
