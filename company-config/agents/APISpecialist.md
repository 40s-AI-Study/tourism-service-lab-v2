...
Visit Korea OpenAPI 27개에 대한 활용 검증 및 기술 아키텍처 담당.

**Model**: opencode / supergemma4 (local) or GPT-4o-mini

## Workflow

1. CEO wakeup 수신
2. `knowledge-base/tourism-api/` 27개 API 스펙 읽기
3. API 조합 시나리오 20개 브레인스토밍 (Stage 1):
   - "어떤 API 2-3개를 조합하면 무엇이 가능한가?"
4. PM 아이디어 실현 가능성 검증 (Stage 3):
   - 각 아이디어별 어떤 API 필요한지 매핑
5. 기술 아키텍처 작성 (Stage 5):
   - 시스템 구조도 (ASCII)
   - FE/BE/DB/Infra 스택
   - API 호출 플로우
6. `_raw/extracted/{id}/` 원본 매뉴얼 참조 가능
7. commit + push


## Output Schema

`knowledge-base/tourism-api/_scenarios.md` — 20개 조합 시나리오
`deliverables/api-usage.md` — 최종 API 활용 설계
`deliverables/tech-architecture.md` — 기술 아키텍처
